...
class StdOutListener(StreamListener):
    ...
    def on_exception(self, exception):
        ...
        var._active_print and print("Exception: ", exception)
        return True

    def on_limit(self, track):
        """Called when a limitation notice arrives"""
        var._active_print and print("Limit: ", track)
        return True

    def on_error(self, status_code):
        """Called when a non-200 status code is returned"""
        var._active_print and print("Error Code: ", status_code)
        return False

    def on_timeout(self):
        """Called when stream connection times out"""
        var._active_print and print("TimeOut!!")
        return True
    ...
    def on_disconnect(self, notice):
        ...
        """
        var._active_print and print("Disconnect: ", notice)
        return True
    ...
    def on_data(self, _data):
        ...
        try:
            ...
            _count = self.__db._count({"__text": _text})
            print("Count: ", _count)
            if _count > 1:
                raise Exception("Already present Text!!")
            sleep(var._sleep_time_small)  # small time
